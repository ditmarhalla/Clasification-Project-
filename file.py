...
def neighbor(traned_data,  untrained_vector):                          # we will call two data data using the fuction of "euclidian_distance"
    distnace_vector = []                                               # Empty list we will use to store the distance and class
    the_class = []                                                     # List we will use to store the class
    ...
    print ("The program runs " ,count, "times")
    return untrained_vector


# test run with some data
number_of_vectors_untrained = 2
dataset_trained = file_to_list("trained.txt")         # Open the untrained data file
dataset_untrained = file_to_list("untrained.txt")

print("The class is",neighbor(dataset_trained,dataset_untrained[0:number_of_vectors_untrained])) # we use slices to call the munber of "untrained" vectors we would like to use
